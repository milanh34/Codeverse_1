    ...
    <div className="w-full fixed top-0 left-0 z-50">
      ...
      <nav
        className={`relative flex items-center justify-between px-6 py-4 w-full transition-all duration-300 ${
          scrolled ? "bg-white shadow-md" : "bg-[#e6f4f2]"
        }`}
      >
        {/* Left - Logo */}
          ...
          <nav className="flex space-x-6 text-gray-700 font-medium">
            {navItems.map((item, index) => (
              <div key={index} className="relative group">
                ...
                >
                  <span className="hover:text-teal-700 transition-all">
                    {item.name}
                  </span>
                  {item.hasDropdown && (
                    <ChevronDown
                      size={16}
                      className={`transition-transform ${activeDropdown === index ? "rotate-180 text-teal-700" : ""}`}
                    />
                  )}
                </div>
